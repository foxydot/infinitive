<?php
/*
Plugin Name: Experts Widget
Plugin URI: http://wordpress.org/extend/plugins/experts-widget/
Description: Based loosely on Shane & Peter's image widget.
*/

// Load the widget on widgets_init
function load_sp_experts_widget() {
	register_widget('MSD_Experts_Widget');
}
add_action('widgets_init', 'load_sp_experts_widget');
 ...
 **/
class MSD_Experts_Widget extends WP_Widget {
	
	var $pluginDomain = 'sp_experts_widget';
	 ...
	 * @author Shane & Peter, Inc. (Peter Chester)
	 */
	function MSD_Experts_Widget() {
		$this->loadPluginTextDomain();
		$widget_ops = array( 'classname' => 'widget_msd_experts', 'description' => __( 'Showcase an expert with a Photo, Title, URL, LinkedIn URL, and a Description', $this->pluginDomain ) );
		$control_ops = array( 'id_base' => 'widget_msd_experts' );
		$this->WP_Widget('widget_msd_experts', __('Experts Widget', $this->pluginDomain), $widget_ops, $control_ops);
		...
		if (defined("WP_ADMIN") && WP_ADMIN) {
			...
			if ( 'widgets.php' == $pagenow ) {
				...
				wp_enqueue_script( $control_ops['id_base'], WP_PLUGIN_URL.'/experts-widget/experts-widget.js',array('thickbox'), false, true );
	 ...
	 * @author Shane & Peter, Inc. (Peter Chester)
	 */
	function is_msd_widget_context() {
	 ...
	 * 2) parent template, 3) plugin resources. will look in the experts-widget/
	...
	function getTemplateHierarchy($template) {
		...
		if ( $theme_file = locate_template(array('experts-widget/'.$template)) ) {
			$file = $theme_file;
		} else {
			$file = 'views/' . $template;
		}
		return apply_filters( 'msd_template_experts_widget_'.$template, $file);
	}
}
?>