...
if ( !current_user_can( 'install_themes' ) ) {
	die( __( 'ACCESS DENIED! You need higher access to do this. Error #534344 PluginBuddy.com.', 'it-l10n-copiouscomments' ) );
}
...
if( is_wp_error( $response ) ) {
	die( sprintf( __( 'ERROR #5455. Unable to access PluginBuddy / iThemes updater & licensing system. Details: %s', 'it-l10n-copiouscomments' ), $response->get_error_message() ) );
} else {
...
if ( is_array( $response_array ) ) { // First line is array data to server.
	...
	if ( isset( $response_array['set_key'] ) ) {
		...
		echo __( 'ERROR: UNKNOWN CALLBACK COMMAND! ERROR #85433256.', 'it-l10n-copiouscomments' );
	}
