			...
			$return .= '<div id="videoshowcaseid-' . $this->_instance . '" class="videoshowcase"' . $alignment . '>';
				...
				for($i=0; $i<$max; $i++){
					...
					if ($video['vsourc'] == 'custom') {
						...
						$return .= '<a href="' . $video['vurl'] . '" rel="' . $this->_var . "-" . $this->_instance . '" title="' . stripslashes($video['vtitle']) . '" width="' . $gpath['width'] . 'px" height="' . $gpath['height'] . 'px" ><img src="' . $imagedata['0'] . '" alt="' . stripslashes($video['vtitle']) . '" /></a>';
					}
					if (isset($gpath['tlink'])) {
